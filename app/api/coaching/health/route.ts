// app/api/coaching/health/route.ts
import { NextRequest, NextResponse } from 'next/server';
import Anthropic from '@anthropic-ai/sdk';
import { createClient } from '@/lib/supabase/server';
import { buildCoachingContext } from '@/lib/coaching';
import { generateHealthCoachPrompt } from '@/lib/coaching/health-coach-prompt';
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Generate health coach prompt
    const systemPrompt = generateHealthCoachPrompt(context);
    ...
    return NextResponse.json({
      ...
      metadata: {
        ...
        totalMeals: context.metrics.homeCookedMeals,
        avgMealsPerDay: context.metrics.avgMealsPerDay,
    ...
    return NextResponse.json(
      { error: 'Failed to generate health coaching', details: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    );
  }
}
